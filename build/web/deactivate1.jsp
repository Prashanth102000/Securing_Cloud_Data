...
<%
    ...
    try {
        ...
        if (rs.next()) {
            ...
            int i = st1.executeUpdate("update oreg set status='No' where id='" + rs.getString("id") + "'");
            if (i != 0) {
//                Mail m = new Mail();
//                m.sendMail("Name :"+rs.getString("name")+"\n\tYou are deactivated by CloudArmor", "KK", rs.getString("email"));
                response.sendRedirect("adowdetails.jsp?dmsg=success");
            } else {
                response.sendRedirect("adowdetails.jsp?dsgg=failed");
            }
        }
        else
        {
